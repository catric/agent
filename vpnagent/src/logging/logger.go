...
var panicFileName = ""
var panicFile *os.File = nil

func writeToPanicLog(s string) {
    
    if panicFile == nil {
        panicFile, _ = os.OpenFile(panicFileName, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
    }
    
    t := timeNow()
    tstr := t.Format("2006-01-02 15:04:05.000")
    
    panicFile.WriteString("[" + tstr + "]" + s + "\n")
}

func (l *Logger) CustomSettings(format string, cus bool, logPath string, logLv string) {
    customFormat = format
    hasCustom = cus
    logFilePath = logPath
    logLevel = logLv
    panicFileName = logFilePath + "_PANIC.log"
}
...
func (l *Logger) log(lvl Level, format string, args ...interface{}) {
    ...
    if hasCustom {
        ...
        filename := logFilePath + "_" + t.Format("2006-01-02") + ".log"
        if logFileName != filename || !fileExists{
        
            fileExists = true
            logFileName = filename
        
            format := MustStringFormatter(customFormat)
            beScreen := NewLogBackend(os.Stdout, "", 0)

            if file != nil {
                file.Close()
            }
            
            lv, _ := LogLevel(logLevel)

            //logfile := "log/nitalk.log"
            file, _ := os.OpenFile(filename, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
            beFile := NewLogBackend(file, "", 0)

            beScreenFormatter := NewBackendFormatter(beScreen, format)
            beFileFormatter := NewBackendFormatter(beFile, format)

            beScreenLeveled := AddModuleLevel(beScreenFormatter)
            beScreenLeveled.SetLevel(lv, "")

            beFileLeveled := AddModuleLevel(beFileFormatter)
            beFileLeveled.SetLevel(lv, "")

            SetBackend(beScreenFormatter, beFileFormatter)
            
            //goid := runtime.GetGoId()
            //format = "["+ strconv.Itoa(goid) + "] " + format
        }
    }
...
func (l *Logger) Panic(args ...interface{}) {
    s := fmt.Sprint(args...)
    l.log(CRITICAL, "%s", s)
    l.log(CRITICAL, "%v", string(debug.Stack()))
    writeToPanicLog(s + "\n" + string(debug.Stack()))
    panic(s)
}
